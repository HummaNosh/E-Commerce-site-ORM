...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({NewCategoryStuff, message: "Good job! A new Category has been created!"});
  } catch (err) {
...
if (CategoryStuff){
  res.status(404).json({ message: 'Good job! This category has been deleted!' });
  return;
}
    if (!CategoryStuff) {
      res.status(404).json({ message: 'No Category found with that id!' });
